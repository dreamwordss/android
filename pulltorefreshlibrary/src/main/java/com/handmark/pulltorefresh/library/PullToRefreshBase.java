...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
    ...
    public PullToRefreshBase setNeedPreLoading(boolean needPreLoading) {
        this.needPreLoading = needPreLoading;
        return this;
    }
    ...
    public boolean onInterceptTouchEvent(MotionEvent event) {
        ...
        switch (action) {
            case MotionEvent.ACTION_MOVE: {
                ...
                if (isReadyForPull()) {
                    ...
                    if (absDiff > mTouchSlop && (!mFilterTouchEvents || absDiff > Math.abs(oppositeDiff))) {
                        if (mMode.showHeaderLoadingLayout() && diff >= 1f && isReadyForPullStart()) {
                            Log.e("pullbase",mMode.showHeaderLoadingLayout()+"start");
                            mLastMotionY = y;
                            mLastMotionX = x;
                            ...
                            Log.e("pullbase",mMode.showHeaderLoadingLayout()+"end"+diff+"--"+isReadyForPullEnd());
                            mLastMotionY = y;
                            mLastMotionX = x;
                            mIsBeingDragged = true;
                            if (mMode == Mode.BOTH) {
                ...
                if(needPreLoading){
                    // TODO: 2016/6/21 处理预加载
                    if (mRefreshableView instanceof AbsListView) {
                        AbsListView mRefreshableView = (AbsListView) this.mRefreshableView;
                        int totalCount = mRefreshableView.getCount();//总共的count
                        int lastVisiblePosition = mRefreshableView.getLastVisiblePosition();//totalCount-1 的时候 是最底端
                        if (totalCount > 4) {
                            if (lastVisiblePosition == (totalCount - 1 - 3)) {
                                if (mOnRefreshListener2 != null) { //开始预加载
                                    mOnRefreshListener2.onPullUpToRefresh(this);
                                }
                            } else if (lastVisiblePosition == totalCount - 1) {//如果当前已经是最后一条
                                if (mOnRefreshListener2 != null) { //开始预加载
                                    mOnRefreshListener2.onPullUpToRefresh(this);
                                }
                            }
...
//      setNeedPreLoading(PullToRefreshBase.Mode.BOTH==mode);
        if (mode != mMode) {
            if (DEBUG) {
    ...
    protected LoadingLayout createLoadingLayout(Context context, Mode mode, TypedArray attrs) {
        ...
        if (mode == Mode.PULL_FROM_END) {//fix foot layout animation
            ImageView mFooterLayoutRefreshView = (ImageView) layout.findViewById(R.id.pull_to_refresh_git);
            mFooterLayoutRefreshView.setImageResource(R.drawable.loading_foot_gif);//replace gif animation
        }
        return layout;
    }
    ...
    private void pullEvent() {
        ...
        if (Math.abs(lastMotionValue - initialMotionValue) >= 20) {
            if (Math.abs(newScrollValue) < itemDimension) {
